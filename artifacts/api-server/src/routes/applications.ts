...
router.post("/jobs/:jobId/applications", async (req, res) => {
  ...
  const [existing] = await db.select({ id: jobApplicationsTable.id }).from(jobApplicationsTable)
    .where(and(eq(jobApplicationsTable.jobId, jobId), eq(jobApplicationsTable.applicantId, req.user!.id))).limit(1);
  if (existing) { res.status(409).json({ error: "لقد تقدمت لهذه الوظيفة من قبل" }); return; }
...
router.get("/jobs/:jobId/applications", async (req, res) => {
  ...
  const rows = await db.select({ application: jobApplicationsTable, applicant: usersTable }).from(jobApplicationsTable)
    .innerJoin(usersTable, eq(jobApplicationsTable.applicantId, usersTable.id)).where(eq(jobApplicationsTable.jobId, req.params.jobId)).orderBy(desc(jobApplicationsTable.createdAt));
...
router.get("/applications/me", async (req, res) => {
  const rows = await db.select({ application: jobApplicationsTable, job: jobsTable }).from(jobApplicationsTable)
    .innerJoin(jobsTable, eq(jobApplicationsTable.jobId, jobsTable.id)).where(eq(jobApplicationsTable.applicantId, req.user!.id)).orderBy(desc(jobApplicationsTable.createdAt));
