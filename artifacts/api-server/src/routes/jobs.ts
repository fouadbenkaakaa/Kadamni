...
// GET /api/jobs?city=&jobType=&limit=&offset=
router.get("/jobs", async (req, res) => {
  ...
  if (jobType) {
    conditions.push(
      eq(
        jobsTable.jobType,
        jobType as "full_time" | "part_time" | "freelance" | "remote",
      ),
    );
  }
  ...
  res.json({
    jobs: rows.map((r) => ({ ...r.job, employer: toPublicUser(r.employer) })),
  });
});

router.get("/jobs/:id", async (req, res) => {
  const [row] = await db
    .select({ job: jobsTable, employer: usersTable })
    .from(jobsTable)
    .innerJoin(usersTable, eq(jobsTable.employerId, usersTable.id))
...
router.post("/jobs", requireAuth, async (req, res) => {
  if (req.user!.accountType !== "employer") {
    res.status(403).json({ error: "نشر الوظائف متاح لأصحاب العمل فقط" });
    return;
  }
  ...
  const [job] = await db
    .insert(jobsTable)
    .values({ ...parsed.data, employerId: req.user!.id })
    .returning();
...
router.patch("/jobs/:id", requireAuth, async (req, res) => {
  ...
  if (existing.employerId !== req.user!.id) {
    res.status(403).json({ error: "لا يمكنك تعديل هذا الإعلان" });
    return;
  }
