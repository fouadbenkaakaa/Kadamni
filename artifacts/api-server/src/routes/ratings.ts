...
router.get("/users/:userId/ratings", async (req, res) => {
  const rows = await db
    ...
    .from(ratingsTable)
    .innerJoin(usersTable, eq(ratingsTable.raterUserId, usersTable.id))
    .where(eq(ratingsTable.targetUserId, req.params.userId))
...
router.post("/users/:userId/ratings", requireAuth, async (req, res) => {
  ...
  if (targetUserId === req.user!.id) {
    res.status(400).json({ error: "لا يمكنك تقييم نفسك" });
    return;
  }
  ...
  if (!parsed.success) {
    res.status(400).json({ error: "التقييم غير صالح" });
    return;
  }
  try {
    const [rating] = await db.insert(ratingsTable).values({ ...parsed.data, raterUserId: req.user!.id }).returning();
    const [aggregate] = await db
      .select({ average: sql<string>`coalesce(avg(${ratingsTable.value}), 0)`, count: sql<number>`count(*)` })
      .from(ratingsTable)
      .where(eq(ratingsTable.targetUserId, targetUserId));
    ...
    if (error?.code === "23505") {
      res.status(409).json({ error: "لقد أرسلت هذا التقييم من قبل" });
      return;
    }
    throw error;
  }
