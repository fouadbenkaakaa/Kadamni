/* ============================================================
   ...
   Organized to mirror a component-based structure so it can be
   ported to React / Vue / any framework later:
     - DATA        : mock/demo data (swap with real API calls)
...
const WORKERS = [
  {
    ...
    dist: "1.8", rating: 4.9, reviews: 256, exp: "5 سنوات", clients: 52,
    done: 120, available: true, price: "80,000",
    bio: "كهربائي محترف بخبرة في التركيبات المنزلية والمشاريع الصناعية، أعمل بدقة وسرعة.",
    skills: ["تركيبات منزلية", "كهرباء صناعية"], seed: 11,
  },
  {
    id: 2, name: "ياسين قادر", job: "سائق نقل سريع", city: "وهران",
    dist: "4.5", rating: 4.5, reviews: 98, exp: "3 سنوات", clients: 34,
    done: 76, available: true, price: "70,000", seed: 12,
  },
  {
    id: 3, name: "كريم نجار", job: "نجار أثاث", city: "قسنطينة",
    dist: "3.2", rating: 4.2, reviews: 61, exp: "6 سنوات", clients: 41,
    done: 88, available: false, price: "60,000", seed: 13,
  },
  {
    id: 4, name: "سامي بلحاج", job: "سباك خبير", city: "البليدة",
    dist: "2.1", rating: 4.7, reviews: 143, exp: "8 سنوات", clients: 66,
    done: 150, available: true, price: "65,000", seed: 14,
  },
];
...
const NOTIFS = [
  { id: 1, type: "job",     title: "وظيفة جديدة تناسب خبرتك",   sub: "كهربائي - شركة البناء الحديث", time: "منذ 5 دقائق",  read: false },
  { id: 2, type: "worker",  title: "أحمد محمد أصبح متاحاً الآن", sub: "على بعد 1.8 كم منك",            time: "منذ 10 دقائق", read: false },
  { id: 3, type: "system",  title: "تم قبول طلبك",               sub: "من قبل شركة البناء الحديث",     time: "منذ 3 ساعات",  read: true  },
  { id: 4, type: "message", title: "رسالة جديدة من أحمد محمد",   sub: "نعم بالتأكيد، أنا متاح",        time: "منذ يوم",       read: true  },
];

const CHATS = [
  {
    id: 1, name: "أحمد محمد", last: "نعم بالتأكيد، أنا متاح", time: "09:31", unread: 2, seed: 11, online: true,
    messages: [
      { from: "them", text: "مرحباً، هل أنت متاح للعمل غداً؟",                          time: "09:30" },
      { from: "me",   text: "نعم بالتأكيد، أنا متاح",                                    time: "09:31" },
      { from: "them", text: "كم تبلغ كلفة الخدمة؟",                                       time: "09:32" },
      { from: "me",   text: "الكلفة 5000 دج وتشمل النقل والمواد الأولية",                 time: "09:34" },
      { from: "them", text: "ممتاز، سأرسل لك موقع العمل",                                 time: "09:35" },
    ],
  },
  {
    id: 2, name: "شركة البناء الحديث", last: "تم قبول طلبك، مرحباً بك", time: "أمس", unread: 0, seed: 30, online: false,
    messages: [{ from: "them", text: "تم قبول طلبك، مرحباً بك في الفريق", time: "أمس" }],
  },
  {
    id: 3, name: "ياسين قادر", last: "الكلفة 5000 دج وتشمل النقل", time: "الإثنين", unread: 0, seed: 12, online: false,
    messages: [{ from: "them", text: "الكلفة 5000 دج وتشمل النقل", time: "الإثنين" }],
  },
  {
    id: 4, name: "سامي بلحاج", last: "صوتية · 0:24", time: "الأحد", unread: 1, seed: 14, online: false,
    messages: [{ from: "them", text: "مرحباً، أرسلت لك رسالة صوتية", time: "الأحد" }],
  },
];
...
function stars(v, size) {
  ...
  return (
    '<div style="display:flex;align-items:center;gap:2px">' +
      icon("star", size, "var(--amber)", 'fill="var(--amber)"') +
      '<span style="font-size:' + (size + 1) + 'px;color:var(--text);font-weight:700">' + v + "</span>" +
    "</div>"
  );
}
    ...
    '<button onclick="go(\'results\')" class="card" style="width:100%;border-radius:16px;padding:14px;display:flex;align-items:center;gap:12px;text-align:right;">' +
      ...
      '<div style="flex:1;min-width:0">' +
        ...
        '<div style="display:flex;align-items:center;gap:8px;margin-top:4px;font-size:10.5px;color:var(--sub)">' +
          '<span style="display:flex;align-items:center;gap:2px">' + icon("map-pin", 10) + " " + j.city + " · " + j.dist + " كم</span>" +
          "<span>" + j.time + "</span>" +
        "</div>" +
      "</div>" +
...
function screenSplash() {
  setTimeout(function() { if (STATE.screen === "splash") go("welcome"); }, 1600);
  return (
  ...
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column">' +
      ...
      '<div class="scroll" style="padding:16px 20px 0;display:flex;flex-direction:column;gap:12px">' +
        field("user",  "الاسم الكامل") +
        field("phone", "رقم الهاتف",           "", 'type="tel"') +
        field("mail",  "البريد الإلكتروني",    "", 'type="email"') +
        field("lock",  "كلمة المرور",          "", 'type="password"') +
        field("lock",  "تأكيد كلمة المرور",   "", 'type="password"') +
  ...
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column">' +
      ...
      '<div style="flex:1;padding:24px 20px 0;display:flex;flex-direction:column;gap:12px">' +
        field("mail", "البريد الإلكتروني أو رقم الهاتف") +
        field("lock", "كلمة المرور", "", 'type="password"') +
...
function screenHome() {
  const availableWorkers = WORKERS.filter(function(w) { return w.available; });
  let workerCards = "";
  ...
  let jobCards = "";
  JOBS.forEach(function(j) { jobCards += jobCard(j); });
        ...
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">' +
          ...
          '<div style="display:flex;align-items:center;gap:8px">' +
            '<button onclick="go(\'notifications\')" class="icon-btn" style="position:relative;background:var(--primarySoft)">' +
              ...
              '<span style="position:absolute;top:6px;left:6px;width:6px;height:6px;border-radius:9999px;background:var(--danger)"></span>' +
            "</button>" +
            '<button onclick="go(\'profile\')" style="background:none;border:none;padding:0;cursor:pointer">' + avatar(12, 34) + "</button>" +
          "</div>" +
        "</div>" +
        '<div style="font-size:13px;color:var(--sub)">مرحباً 👋 أحمد</div>' +
...
function screenSearch() {
  ...
  let results = "";
  if (tab === "all" || tab === "jobs") {
    JOBS.forEach(function(j) { results += jobCard(j); });
  }
      ...
      results +=
        '<button onclick="go(\'worker-profile\', ' + w.id + ')" class="card" style="width:100%;border-radius:16px;padding:12px;display:flex;align-items:center;gap:12px;text-align:right;cursor:pointer">' +
          ...
          '<div style="flex:1;min-width:0">' +
            ...
            stars(w.rating, 11) +
          "</div>" +
          '<div style="font-weight:900;font-size:12px;color:var(--primaryDark)">' + w.price + " دج</div>" +
        "</button>";
    });
  }
  return (
  ...
  return (
    '<div class="sheet-overlay">' +
      ...
      '<div class="sheet fade-in">' +
        ...
        '<div class="scroll" style="padding:16px 20px;display:flex;flex-direction:column;gap:20px">' +
          "<div>" +
            '<div style="font-size:12.5px;font-weight:700;margin-bottom:8px;color:var(--text)">المدينة</div>' +
            '<div style="display:flex;align-items:center;justify-content:space-between;border-radius:16px;padding:0 14px;height:44px;background:var(--primarySofter)">' +
              '<span style="font-size:13px;color:var(--text)">الجزائر العاصمة</span>' + icon("chevron-down", 16, "var(--sub)") +
            "</div>" +
          "</div>" +
          "<div>" +
            '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">' +
              '<span style="font-size:12.5px;font-weight:700;color:var(--text)">بالقرب مني</span>' +
              '<span id="dist-val" style="font-size:12px;font-weight:700;color:var(--primary)">10 كم</span>' +
            "</div>" +
            '<input type="range" min="1" max="50" value="10" style="width:100%" oninput="document.getElementById(\'dist-val\').textContent=this.value+\' كم\'"/>' +
          "</div>" +
          "<div>" +
            '<div style="font-size:12.5px;font-weight:700;margin-bottom:8px;color:var(--text)">المهنة</div>' +
            '<div style="display:flex;align-items:center;justify-content:space-between;border-radius:16px;padding:0 14px;height:44px;background:var(--primarySofter)">' +
              '<span style="font-size:13px;color:var(--text)">كهربائي</span>' + icon("chevron-down", 16, "var(--sub)") +
            "</div>" +
          "</div>" +
          "<div>" +
            '<div style="font-size:12.5px;font-weight:700;margin-bottom:8px;color:var(--text)">نوع العمل</div>' +
            '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
              (function() {
                const types = ["دوام كامل", "دوام جزئي", "عمل حر", "عن بعد"];
                return types.map(function(l, i) {
                  return '<button class="chip" style="background:' + (i === 0 ? "var(--primary)" : "var(--primarySoft)") + ";color:" + (i === 0 ? "#fff" : "var(--primaryDark)") + ';height:36px">' + l + "</button>";
                }).join("");
              })() +
            "</div>" +
          "</div>" +
          "<div>" +
            '<div style="font-size:12.5px;font-weight:700;margin-bottom:8px;color:var(--text)">الخبرة</div>' +
            '<div style="display:flex;align-items:center;justify-content:space-between;border-radius:16px;padding:0 14px;height:44px;background:var(--primarySofter)">' +
              '<span style="font-size:13px;color:var(--text)">من 2 إلى 5 سنوات</span>' + icon("chevron-down", 16, "var(--sub)") +
            "</div>" +
          "</div>" +
          "<div>" +
            '<div style="font-size:12.5px;font-weight:700;margin-bottom:8px;color:var(--text)">الراتب</div>' +
            '<div style="display:flex;align-items:center;justify-content:space-between;border-radius:16px;padding:0 14px;height:44px;background:var(--primarySofter)">' +
              '<span style="font-size:13px;color:var(--text)">حتى 100,000 دج</span>' +
            "</div>" +
          "</div>" +
          '<div style="display:flex;flex-direction:column;gap:10px">' +
            '<label style="display:flex;align-items:center;justify-content:space-between"><span style="font-size:12.5px;color:var(--text)">متاح الآن فقط</span><input type="checkbox"/></label>' +
            '<label style="display:flex;align-items:center;justify-content:space-between"><span style="font-size:12.5px;color:var(--text)">الأعلى تقييماً</span><input type="checkbox"/></label>' +
          "</div>" +
        "</div>" +
        '<div style="padding:16px 20px;flex-shrink:0;border-top:1px solid var(--border)">' + btn("تطبيق الفلاتر", "closeSheet()") + "</div>" +
      "</div>" +
    "</div>"
  );
}

function findWorker(id) { return WORKERS.find(function(w) { return w.id === id; }) || WORKERS[0]; }

function screenWorkerProfile() {
  const w = typeof STATE.payload === "number" ? findWorker(STATE.payload) : (STATE.payload || WORKERS[0]);
  const fav = STATE.favorites.includes(w.id);
  const stats = [["الخبرة", w.exp], ["عميل", w.clients], ["عمل منجز", w.done]];
  let statsHtml = "";
  stats.forEach(function(item, i) {
    statsHtml +=
      '<div style="flex:1;padding:10px 0;' + (i < 2 ? "border-left:1px solid var(--border)" : "") + '">' +
  ...
  let portfolioHtml = "";
  for (let i = 1; i <= 3; i++) {
    portfolioHtml += '<img src="https://picsum.photos/seed/' + w.id + i + '/200/200" style="width:100%;aspect-ratio:1;object-fit:cover;border-radius:12px"/>';
  }
  const skillsList = (w.skills || ["احترافية", "التزام بالمواعيد"]);
  let skillsHtml = "";
  ...
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      topBar("الملف الشخصي", true,
        '<button class="icon-btn" onclick="toggleFav(' + w.id + ')">' + icon("heart", 19, fav ? "#EF4444" : "var(--sub)", fav ? 'fill="#EF4444"' : "") + "</button>"
      ) +
      '<div class="scroll">' +
        ...
        '<div style="padding:16px 20px;display:flex;flex-direction:column;gap:16px">' +
          "<div>" +
            ...
            '<p style="font-size:12.5px;line-height:1.7;color:var(--sub)">' + (w.bio || "عامل محترف يقدم خدمات عالية الجودة بالتزام تام بالمواعيد وجودة العمل.") + "</p>" +
          "</div>" +
          "<div>" +
          ...
          "</div>" +
          "<div>" +
            '<div style="font-weight:700;font-size:13.5px;margin-bottom:8px;color:var(--text)">معرض الأعمال</div>' +
            '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">' + portfolioHtml + "</div>" +
          "</div>" +
        "</div>" +
      "</div>" +
      '<div style="padding:14px 20px;display:flex;gap:12px;flex-shrink:0;background:var(--card);border-top:1px solid var(--border)">' +
        btn(icon("message-circle", 16) + " دردشة", "go('chat', " + w.id + ")", "outline") +
...
function screenResults() {
      ...
      '<div class="card" style="border-radius:16px;padding:14px;display:flex;flex-direction:column;gap:8px">' +
        '<button onclick="go(\'worker-profile\', ' + w.id + ')" style="display:flex;align-items:center;gap:12px;text-align:right;background:none;border:none;cursor:pointer;width:100%">' +
          ...
          '<div style="flex:1;min-width:0">' +
            ...
            '<div style="display:flex;align-items:center;gap:8px;margin-top:4px">' + stars(w.rating, 11) + '<span style="font-size:10.5px;color:var(--sub)">' + w.dist + " كم · " + w.exp + "</span></div>" +
          "</div>" +
          '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">' +
            '<span onclick="event.stopPropagation();toggleFav(' + w.id + ')">' + icon("heart", 16, fav ? "#EF4444" : "var(--sub)", fav ? 'fill="#EF4444"' : "") + "</span>" +
        ...
        "</button>" +
        '<div style="display:flex;gap:8px">' +
          btn(icon("message-circle", 13) + " دردشة", "go('chat', " + w.id + ")", "ghost",   "", 'style="height:36px;font-size:12px;flex:1"') +
  ...
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      ...
      '<div style="padding:12px 20px 4px;font-size:11.5px;color:var(--sub)">' + WORKERS.length + " نتيجة</div>" +
      '<div class="scroll" style="padding:12px 20px;display:flex;flex-direction:column;gap:12px">' + workerCards + "</div>" +
    "</div>"
  );
}
...
function screenNearby() {
  ...
  pins.forEach(function(p, i) {
    ...
    pinHtml +=
      '<button onclick="go(\'worker-profile\', ' + w.id + ')" style="position:absolute;transform:translate(-50%,-100%);left:' + p.x + ";top:" + p.y + ';background:none;border:none;cursor:pointer">' +
  ...
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      ...
      '<div style="position:relative;flex:1;background:linear-gradient(135deg,#EAF7EE,#DDF3E4)">' +
        ...
        pinHtml +
      "</div>" +
      '<div style="padding:12px 16px;flex-shrink:0;background:var(--card);border-top:1px solid var(--border)">' +
        '<button onclick="go(\'worker-profile\', 1)" style="display:flex;align-items:center;gap:12px;width:100%;border-radius:16px;padding:10px;background:var(--primarySofter);border:none;cursor:pointer">' +
          avatar(11, 42, true) +
          '<div style="flex:1;text-align:right">' +
            '<div style="font-weight:700;font-size:12.5px;color:var(--text)">كهربائي - أحمد محمد</div>' +
            '<div style="font-size:11px;color:var(--sub)">4.9 ★ (256) · 1.8 كم</div>' +
          "</div>" +
          '<span style="font-size:10px;font-weight:700;padding:4px 8px;border-radius:9999px;background:#DCFCE7;color:#16A34A">متاح الآن</span>' +
        "</button>" +
      "</div>" +
    "</div>"
  );
  ...
  STATE.chats.forEach(function(c) {
    chatList +=
      '<button onclick="go(\'chat\', ' + c.id + ')" style="width:100%;display:flex;align-items:center;gap:12px;padding:12px 20px;text-align:right;background:none;border:none;border-bottom:1px solid var(--border);cursor:pointer">' +
        avatar(c.seed, 48, c.online) +
        '<div style="flex:1;min-width:0">' +
          '<div style="display:flex;align-items:center;justify-content:space-between">' +
            '<span style="font-weight:700;font-size:13px;color:var(--text)">' + c.name + "</span>" +
            '<span style="font-size:10.5px;color:' + (c.unread ? "var(--primary)" : "var(--sub)") + '">' + c.time + "</span>" +
          "</div>" +
          '<div style="display:flex;align-items:center;justify-content:space-between;margin-top:2px">' +
            '<span style="font-size:12px;color:' + (c.unread ? "var(--text)" : "var(--sub)") + ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px">' + c.last + "</span>" +
            (c.unread > 0 ? '<span style="width:20px;height:20px;border-radius:9999px;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;color:#fff;flex-shrink:0;background:var(--primary)">' + c.unread + "</span>" : "") +
          "</div>" +
        "</div>" +
      "</button>";
  });
...
function screenChat() {
  const chatId = typeof STATE.payload === "number" ? STATE.payload : ((STATE.payload && STATE.payload.id) || 1);
  const c = STATE.chats.find(function(x) { return x.id === chatId; }) || STATE.chats[0];
  let messages = "";
  c.messages.forEach(function(m) {
    messages +=
      '<div style="max-width:75%;display:flex;flex-direction:column;align-self:' + (m.from === "me" ? "flex-end" : "flex-start") + ";align-items:" + (m.from === "me" ? "flex-end" : "flex-start") + '">' +
        '<div style="border-radius:16px;padding:10px 14px;font-size:12.5px;line-height:1.5;' +
          ...
          "border-bottom-left-radius:" + (m.from === "me" ? "4px" : "16px") + ";" +
          'border-bottom-right-radius:' + (m.from === "me" ? "16px" : "4px") + '">' + m.text + "</div>" +
        '<span style="font-size:9.5px;margin-top:4px;color:var(--sub)">' + m.time + "</span>" +
      "</div>";
  });
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      '<div style="display:flex;align-items:center;gap:12px;padding:0 16px;height:56px;flex-shrink:0;background:var(--card);border-bottom:1px solid var(--border)">' +
        ...
        avatar(c.seed, 36, c.online) +
        '<div style="flex:1">' +
          '<div style="font-weight:700;font-size:13px;color:var(--text)">' + c.name + "</div>" +
          '<div style="font-size:10.5px;color:' + (c.online ? "#16A34A" : "var(--sub)") + '">' + (c.online ? "متصل الآن" : "غير متصل") + "</div>" +
        "</div>" +
        '<button class="icon-btn" style="background:var(--primarySoft)" onclick="go(\'call\', ' + c.id + ')">' + icon("phone", 16, "var(--primaryDark)") + "</button>" +
      "</div>" +
      '<div class="scroll" id="chat-scroll" style="padding:16px;display:flex;flex-direction:column;gap:10px">' +
        messages +
        '<div class="card" style="align-self:flex-start;border-radius:16px;padding:10px 14px;display:flex;align-items:center;gap:8px">' +
          icon("map-pin", 14, "var(--primaryDark)") + '<span style="font-size:11.5px;color:var(--text)">تم إرسال الموقع</span>' +
        "</div>" +
      "</div>" +
      '<div style="display:flex;align-items:center;gap:8px;padding:10px 12px;flex-shrink:0;background:var(--card);border-top:1px solid var(--border)">' +
        '<button class="icon-btn">' + icon("paperclip", 19, "var(--sub)") + "</button>" +
        '<button class="icon-btn">' + icon("image", 19, "var(--sub)") + "</button>" +
        '<input id="chat-input" placeholder="اكتب رسالتك..." onkeydown="if(event.key===\'Enter\') sendChat(' + c.id + ')" style="flex:1;height:40px;border-radius:9999px;padding:0 16px;font-size:12.5px;border:none;background:var(--primarySofter);color:var(--text)"/>' +
        '<button class="icon-btn" style="background:var(--primary)" onclick="sendChat(' + c.id + ')">' + icon("send", 16, "#fff") + "</button>" +
      "</div>" +
    "</div>"
  );
}
...
function sendChat(chatId) {
  ...
  if (!c) return;
  c.messages.push({ from: "me", text: input.value.trim(), time: "الآن" });
  c.last = input.value.trim();
  c.time = "الآن";
  render();
...
function screenCall() {
  const id = typeof STATE.payload === "number" ? STATE.payload : ((STATE.payload && STATE.payload.id) || 1);
  const c = STATE.chats.find(function(x) { return x.id === id; }) || { name: "أحمد محمد", seed: 11, id: 1 };
  const mm = String(Math.floor(STATE.callSeconds / 60)).padStart(2, "0");
  const ss = String(STATE.callSeconds % 60).padStart(2, "0");
  return (
    '<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:space-between;padding:56px 32px;background:linear-gradient(180deg,#0F2A1B,#0A1F14)">' +
      '<div style="display:flex;flex-direction:column;align-items:center">' +
        '<img src="https://i.pravatar.cc/160?img=' + (c.seed || 11) + '" style="width:112px;height:112px;border-radius:9999px;object-fit:cover;border:4px solid rgba(255,255,255,.1)"/>' +
        '<div style="color:#fff;font-weight:700;font-size:18px;margin-top:16px">' + c.name + "</div>" +
      ...
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px">' +
        ...
        "</button>" +
      "</div>" +
      '<button onclick="go(\'chat\', ' + (c.id || 1) + ')" style="width:64px;height:64px;border-radius:9999px;display:flex;align-items:center;justify-content:center;border:none;cursor:pointer;background:var(--danger)">' + icon("phone-off", 26, "#fff") + "</button>" +
    "</div>"
  );
}
      ...
      '<div style="padding:20px 20px 16px;flex-shrink:0;background:var(--card);border-bottom:1px solid var(--border)">' +
        '<div style="display:flex;align-items:center;gap:12px">' +
          '<div style="position:relative">' +
            avatar(12, 58) +
            '<button style="position:absolute;bottom:-4px;left:-4px;width:24px;height:24px;border-radius:9999px;display:flex;align-items:center;justify-content:center;border:2px solid #fff;background:var(--primary)">' + icon("camera", 11, "#fff") + "</button>" +
          "</div>" +
          '<div style="flex:1">' +
            '<div style="display:flex;align-items:center;gap:4px">' +
              '<span style="font-weight:900;font-size:15px;color:var(--text)">أحمد محمد</span>' + icon("badge-check", 15, "var(--primary)") +
            "</div>" +
            '<div style="font-size:11.5px;color:var(--sub)">باحث عن عمل</div>' +
          "</div>" +
        ...
        "</div>" +
        '<div style="margin-top:12px;display:flex;align-items:center;justify-content:space-between;font-size:11px;color:var(--sub)">' +
          '<span>اكتمال الملف الشخصي</span><span style="font-weight:700;color:var(--primary)">85%</span>' +
        "</div>" +
        '<div style="height:6px;border-radius:9999px;margin-top:4px;background:var(--border)"><div style="height:100%;width:85%;border-radius:9999px;background:var(--primary)"></div></div>' +
      "</div>" +
      '<div class="scroll" style="padding:16px 20px">' +
        '<div class="card" style="border-radius:16px;overflow:hidden">' + menuItems + "</div>" +
        '<button onclick="go(\'welcome\')" style="width:100%;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:20px;padding:12px;border-radius:16px;font-weight:700;font-size:13px;border:none;cursor:pointer;color:#EF4444;background:#FEF2F2">' +
...
function screenPostJob() {
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      ...
      '<div class="scroll" style="padding:16px 20px;display:flex;flex-direction:column;gap:12px">' +
        field("", "عنوان الوظيفة") +
        '<textarea placeholder="وصف الوظيفة" rows="4" style="border-radius:16px;padding:12px 14px;font-size:13px;background:var(--primarySofter);border:1px solid var(--border);color:var(--text)"></textarea>' +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' + field("dollar-sign", "الراتب") + field("map-pin", "المدينة") + "</div>" +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' + field("", "عدد العمال المطلوبين") + field("", "سنوات الخبرة") + "</div>" +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' +
          field("", "العمر") +
          '<div class="field"><select style="width:100%;background:transparent;border:none;font-size:13px;color:var(--text)"><option>الجنس - غير محدد</option><option>ذكر</option><option>أنثى</option></select></div>' +
        "</div>" +
        '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
          (function() {
            return ["دوام كامل", "دوام جزئي", "عمل حر", "عن بعد"].map(function(t, i) {
              return '<button class="chip" style="background:' + (i === 0 ? "var(--primary)" : "var(--primarySoft)") + ";color:" + (i === 0 ? "#fff" : "var(--primaryDark)") + ';height:36px">' + t + "</button>";
            }).join("");
          })() +
        "</div>" +
        field("clock", "تاريخ بداية العمل") +
        '<button style="border-radius:16px;height:96px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;background:var(--primarySofter);border:1.5px dashed var(--border);cursor:pointer">' +
          icon("camera", 20, "var(--sub)") + '<span style="font-size:11.5px;color:var(--sub)">رفع صور</span>' +
        "</button>" +
      "</div>" +
      '<div style="padding:16px 20px;flex-shrink:0;background:var(--card);border-top:1px solid var(--border)">' + btn("نشر الإعلان", "go('home')") + "</div>" +
    "</div>"
  );
}
...
function screenPostService() {
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      ...
      '<div class="scroll" style="padding:16px 20px;display:flex;flex-direction:column;gap:12px">' +
        field("", "اسم الخدمة") +
        '<textarea placeholder="وصف الخدمة" rows="4" style="border-radius:16px;padding:12px 14px;font-size:13px;background:var(--primarySofter);border:1px solid var(--border);color:var(--text)"></textarea>' +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' + field("dollar-sign", "السعر") + field("clock", "مدة الإنجاز") + "</div>" +
        field("map-pin", "المدينة") +
        '<label style="display:flex;align-items:center;justify-content:space-between;border-radius:16px;padding:0 14px;height:48px;background:var(--primarySofter);border:1px solid var(--border)">' +
          '<span style="font-size:13px;color:var(--text)">إمكانية التنقل للعميل</span><input type="checkbox" checked/>' +
        "</label>" +
        '<label style="display:flex;align-items:center;justify-content:space-between;border-radius:16px;padding:0 14px;height:48px;background:var(--primarySofter);border:1px solid var(--border)">' +
          '<span style="font-size:13px;color:var(--text)">متاح الآن</span><input type="checkbox" checked/>' +
        "</label>" +
        '<div style="font-size:12.5px;font-weight:700;color:var(--text)">معرض صور الأعمال</div>' +
        '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">' +
          '<div style="aspect-ratio:1;border-radius:12px;background:var(--primarySoft)"></div>' +
          '<div style="aspect-ratio:1;border-radius:12px;background:var(--primarySoft)"></div>' +
          '<button style="aspect-ratio:1;border-radius:12px;display:flex;align-items:center;justify-content:center;border:1.5px dashed var(--border);background:none;cursor:pointer">' + icon("plus", 18, "var(--sub)") + "</button>" +
        "</div>" +
      "</div>" +
      '<div style="padding:16px 20px;flex-shrink:0;background:var(--card);border-top:1px solid var(--border)">' + btn("نشر الخدمة", "go('home')") + "</div>" +
    "</div>"
  );
}
...
function screenNotifications() {
  ...
  let notifList = "";
  STATE.notifs.filter(function(n) { return tab === "all" || n.type === tab; }).forEach(function(n) {
    notifList +=
      '<div style="display:flex;align-items:center;gap:12px;padding:14px 20px;border-bottom:1px solid var(--border);background:' + (n.read ? "transparent" : "var(--primarySofter)") + '">' +
        ...
        '<div style="flex:1;min-width:0">' +
          ...
          '<div style="font-size:11.5px;color:var(--sub);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + n.sub + "</div>" +
          '<div style="font-size:10px;margin-top:2px;color:var(--sub)">' + n.time + "</div>" +
        "</div>" +
...
function screenFavorites() {
  const list = WORKERS.filter(function(w) { return STATE.favorites.includes(w.id); });
  let items = "";
    ...
    items +=
      '<button onclick="go(\'worker-profile\', ' + w.id + ')" class="card" style="width:100%;border-radius:16px;padding:12px;display:flex;align-items:center;gap:12px;text-align:right;cursor:pointer">' +
        ...
        '<div style="flex:1;min-width:0">' +
          ...
          stars(w.rating, 11) +
        "</div>" +
        '<span onclick="event.stopPropagation();toggleFav(' + w.id + ')">' + icon("heart", 17, "#EF4444", 'fill="#EF4444"') + "</span>" +
      "</button>";
  });
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      topBar("المفضلة", true) +
      '<div class="scroll" style="padding:16px 20px;display:flex;flex-direction:column;gap:12px">' +
        (list.length === 0 ? '<div style="text-align:center;font-size:12.5px;margin-top:40px;color:var(--sub)">لا توجد عناصر محفوظة بعد</div>' : "") +
        items +
      "</div>" +
    "</div>"
  );
    ...
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      ...
      '<div class="scroll" style="padding:16px 20px;display:flex;flex-direction:column;gap:12px">' +
        '<div style="display:flex;flex-direction:column;align-items:center;margin-bottom:8px">' +
          '<div style="position:relative">' +
            avatar(12, 72) +
            '<button style="position:absolute;bottom:-4px;left:-4px;width:28px;height:28px;border-radius:9999px;display:flex;align-items:center;justify-content:center;border:2px solid #fff;background:var(--primary)">' + icon("camera", 13, "#fff") + "</button>" +
          "</div>" +
        "</div>" +
        field("user",      "الاسم الكامل",       "", "", "أحمد محمد") +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' +
          field("", "العمر", "", "", "29") +
          '<div class="field"><select style="width:100%;background:transparent;border:none;font-size:13px;color:var(--text)"><option>ذكر</option><option>أنثى</option></select></div>' +
        "</div>" +
        field("map-pin",   "المدينة",            "", "", "الجزائر العاصمة") +
        field("briefcase", "المهنة",             "", "", "كهربائي محترف") +
        field("award",     "سنوات الخبرة",       "", "", "5 سنوات") +
        field("languages", "اللغات",             "", "", "العربية، الفرنسية") +
        field("phone",     "رقم الهاتف",         "", "", "0555 12 34 56") +
        field("mail",      "البريد الإلكتروني",  "", "", "ahmed@mail.com") +
      "</div>" +
      '<div style="padding:16px 20px;flex-shrink:0;background:var(--card);border-top:1px solid var(--border)">' + btn("حفظ التعديلات", "go('profile')") + "</div>" +
    "</div>"
  );
}
  ...
  return (
    '<div style="background:var(--bg);flex:1;display:flex;flex-direction:column;overflow:hidden">' +
      ...
      '<div class="scroll" style="padding:16px 20px;display:flex;flex-direction:column;gap:16px">' +
        ...
        "</div>" +
        '<div class="card" style="border-radius:16px;overflow:hidden">' +
          '<button style="width:100%;display:flex;align-items:center;gap:12px;padding:14px 16px;background:none;border:none;cursor:pointer;text-align:right">' +
            '<div style="width:36px;height:36px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:#FEF2F2">' + icon("trash-2", 16, "#EF4444") + "</div>" +
            '<span style="flex:1;text-align:right;font-size:13px;font-weight:600;color:#EF4444">حذف الحساب</span>' +
          "</button>" +
        "</div>" +
